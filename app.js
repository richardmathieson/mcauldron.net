(function() {
  ...
  const categories = [
    { key: 'awards', label: 'Awards', synonyms: ['award','awards'], codePrefix: 'AWD', codeRegex: /\bAWD-\d{3,6}\b/gi },
    { key: 'offers', label: 'Offers', synonyms: ['offer','offers'], codePrefix: 'OFF', codeRegex: /\bOFF-\d{3,6}\b/gi },
    { key: 'bid-events', label: 'Bid Events', synonyms: ['bid','bids','tender','rfq'], codePrefix: 'BID', codeRegex: /\bBID-\d{3,6}\b/gi },
    { key: 'teva', label: 'Teva', synonyms: ['teva'], codePrefix: 'TEVA', codeRegex: /\bTEVA-\d{3,6}\b/gi },
    { key: 'hikma', label: 'Hikma', synonyms: ['hikma'], codePrefix: 'HIKMA', codeRegex: /\bHIKMA-\d{3,6}\b/gi },
    { key: 'camber', label: 'Camber', synonyms: ['camber'], codePrefix: 'CAMBER', codeRegex: /\bCAMBER-\d{3,6}\b/gi },
    { key: 'price-changes', label: 'Price Changes', synonyms: ['price change','price changes','pricing','price'], codePrefix: 'PRC', codeRegex: /\bPRC-\d{4}-\d{1,2}\b/gi },
    { key: 'negotiations', label: 'Negotiations', synonyms: ['negotiation','negotiations','negotiate'], codePrefix: 'NEG', codeRegex: /\bNEG-\d{3,6}\b/gi },
    { key: 'negotiation-items', label: 'Negotiation Items', synonyms: ['negotiation item','line item','item'], codePrefix: 'NEGI', codeRegex: /\bNEGI-\d{3,6}\b/gi },
    // New categories
    { key: 'ndc', label: 'NDC', synonyms: ['ndc'], codePrefix: 'NDC', codeRegex: /\b\d{9}\b/gi },
    { key: 'gid', label: 'GID', synonyms: ['gid'], codePrefix: 'GID', codeRegex: /\bGID-?\d{5}\b/gi },
    { key: 'fid', label: 'FID', synonyms: ['fid'], codePrefix: 'FID', codeRegex: /\bFID-?\d{4}\b/gi }
  ];
  ...
  const catalog = [
    ...
    // Legacy numeric-like issues (for #4712 refs)
  ...
  function setBulkButtonsState() {
    ...
    suggestionsAside.classList.toggle('is-hidden', !anySuggestions && activeLinkIds.size === 0);
  }

  function renderColumns() {
    categoryColumnsEl.innerHTML = '';
    const keys = Array.from(currentCategorySuggestions.keys());
    const ordered = categories.map(c => c.key).filter(k => keys.includes(k));
  ...
  function extractExplicitRefs(text) {
    ...
    for (const c of categories) { const re = new RegExp(c.codeRegex); let mm; while ((mm = re.exec(text)) !== null) ids.add(mm[0].toUpperCase()); }
  ...
  function scoreCatalogItemAgainst(text, item) {
    ...
    // Bias most results above 0.8 when category synonyms are present
  ...
  function generateCategorySuggestions(text) {
    ...
    // 1) Explicit: exact matches from catalog
    ...
    // 2) Explicit: dynamic codes (NDC/GID/FID/etc.)
    ...
    // 3) Category keyword present -> surface multiple items from that category immediately
    for (const c of categories) {
      const hasSyn = c.synonyms.some(s => t.includes(s));
      const hasCodeMention = (t.match(c.codeRegex) || []).length > 0;
      if (hasSyn || hasCodeMention) {
        const base = hasCodeMention ? 0.95 : 0.84; // lift above 80 by default
    ...
    // 4) General relevance-based fill for remaining categories
  ...
  function acceptAllSuggestions() {
    // Accept only items at or above threshold; keep them visible in columns
    for (const arr of currentCategorySuggestions.values()) {
